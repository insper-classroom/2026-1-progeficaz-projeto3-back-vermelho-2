#!/usr/bin/env python
"""
Teste de ESPECIALIZAR contra MongoDB Atlas REAL
"""
import os
import sys
import json
...
def test_especializar_atlas(curso: str):
    print('='*70)
    print(f'🔍 TESTE ATLAS: Especializar "{curso}"')
    print('='*70)
    ...
    try:
        ...
        if existing:
            ...
            print(f'\n💡 Cursos disponíveis:')
            for doc in db_service.responses.find({'category': 'especializar'}):
                key = doc["value_key"]
                if key:
                    print(f'   • {key}')
            db_service.close()
...
def main():
    print('\n🚀 TESTE DE ESPECIALIZAR COM MONGODB ATLAS (Real)\n')
    
    if len(sys.argv) < 2:
        print('📋 Uso: python test_especializar_atlas.py "Curso"\n')
        print('Exemplos:')
        print('  python test_especializar_atlas.py "Engenharia"')
        print('  python test_especializar_atlas.py "Ciencia da Computacao"\n')
        sys.exit(0)
    
    curso = sys.argv[1]
    test_especializar_atlas(curso)
    print('\n' + '='*70)
